...
$stmt = $db->prepare("SELECT * FROM surat_masuk WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $file_sql = "";
    $params = [$nomor_surat, $tanggal, $pengirim, $perihal];
    $types = "ssss";
    ...
    if (empty($error)) {
        $params[] = $id;
        $types .= "i";
        $stmt = $db->prepare("UPDATE surat_masuk SET nomor_surat=?, tanggal=?, pengirim=?, perihal=? $file_sql WHERE id=?");
        $stmt->bind_param($types, ...$params);
        if ($stmt->execute()) {
            echo "<script>alert('Data berhasil diupdate.'); window.location.href='admin.php?page=surat_masuk';</script>";
            exit;
        } else {
...
<div class="max-w-2xl mx-auto bg-white shadow-lg rounded-xl p-8">
    ...
    <form method="post" enctype="multipart/form-data" class="space-y-5">
        <div>
            ...
            <input type="text" id="nomor_surat" name="nomor_surat" value="<?= htmlspecialchars($data['nomor_surat']) ?>" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
        </div>
        <div>
            <label for="tanggal" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Surat</label>
            <input type="date" id="tanggal" name="tanggal" value="<?= htmlspecialchars($data['tanggal']) ?>" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
        </div>
        <div>
            <label for="pengirim" class="block text-sm font-medium text-gray-700 mb-1">Pengirim</label>
            <input type="text" id="pengirim" name="pengirim" value="<?= htmlspecialchars($data['pengirim']) ?>" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
        </div>
        <div>
            <label for="perihal" class="block text-sm font-medium text-gray-700 mb-1">Perihal</label>
            <textarea id="perihal" name="perihal" rows="3" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500" required><?= htmlspecialchars($data['perihal']) ?></textarea>
        </div>
        <div>
            <label for="file_surat" class="block text-sm font-medium text-gray-700 mb-1">Ganti File Surat (Opsional)</label>
            <input type="file" id="file_surat" name="file_surat" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
