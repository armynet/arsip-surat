...
$data_surat_masuk = $db->query("SELECT * FROM surat_masuk ORDER BY tanggal DESC");

// Logika untuk hapus data
if (isset($_GET['action']) && $_GET['action'] == 'hapus' && isset($_GET['id'])) {
    $id_hapus = $_GET['id'];
    // Hapus file terkait jika ada
    $file_stmt = $db->prepare("SELECT file_surat FROM surat_masuk WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        header('Location: admin.php?page=surat_masuk&status=success_delete');
        exit;
    } else {
        // Menggunakan parameter URL untuk pesan error
        header('Location: admin.php?page=surat_masuk&status=error_delete');
        exit;
    }
    $stmt->close();
}
...
<div class="bg-white rounded-2xl shadow-xl p-8 border border-blue-100">
    <div class="flex flex-col sm:flex-row items-center justify-between mb-8">
        <h1 class="text-3xl font-bold text-gray-800 flex items-center gap-3 mb-4 sm:mb-0">
            <span class="iconify text-blue-600 text-4xl" data-icon="mdi:inbox-arrow-down-outline"></span>
            Data Surat Masuk
        </h1>
        <a href="admin.php?page=tambah_surat_masuk" class="bg-gradient-to-r from-blue-500 to-indigo-600 text-white px-6 py-3 rounded-full shadow-lg hover:from-blue-600 hover:to-indigo-700 transition-all duration-300 flex items-center gap-2 text-lg font-semibold">
    ...
    <div class="overflow-x-auto rounded-lg shadow-md">
        <table class="w-full text-base text-left text-gray-700">
            ...
            <tbody class="text-gray-800 divide-y divide-gray-200">
                <?php if ($data_surat_masuk && $data_surat_masuk->num_rows > 0) : ?>
                    <?php $no = 1; ?>
                    <?php while ($row = $data_surat_masuk->fetch_assoc()) : ?>
                        <tr class="hover:bg-blue-50/50 transition-colors duration-200">
                            ...
                            <td class="py-3 px-6"><?= htmlspecialchars($row['pengirim']); ?></td>
                            <td class="py-3 px-6"><?= htmlspecialchars($row['perihal']); ?></td>
                            <td class="py-3 px-6 text-center">
                                <a href="uploads/<?= htmlspecialchars($row['file_surat']); ?>" target="_blank" class="bg-gradient-to-r from-blue-500 to-indigo-500 text-white px-4 py-2 rounded-full text-sm font-semibold shadow-md hover:from-blue-600 hover:to-indigo-600 transition-all transform hover:scale-105">
                                    Lihat
                                </a>
                            </td>
                            <td class="py-3 px-6 text-center flex items-center justify-center gap-3">
                                <a href="admin.php?page=edit_surat_masuk&id=<?= $row['id']; ?>" class="text-yellow-500 hover:text-yellow-700 transition-colors duration-200" title="Edit">
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="7" class="text-center text-gray-500 py-10 text-lg">Tidak ada data surat masuk.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<script>
    ...
    confirmBtn.addEventListener('click', () => {
        if (currentDeleteId) {
            window.location.href = `admin.php?page=surat_masuk&action=hapus&id=${currentDeleteId}`;
        }
    });
